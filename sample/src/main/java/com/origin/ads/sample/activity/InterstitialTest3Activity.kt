...
import com.origin.ads.sample.databinding.ActivityInterstitialTest3Binding
import com.origin.ads.sample.utils.getInterAdsUnitId

class InterstitialTest3Activity : AppCompatActivity() {
    private lateinit var mActivityInterstitialTest3Binding: ActivityInterstitialTest3Binding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mActivityInterstitialTest3Binding = ActivityInterstitialTest3Binding.inflate(layoutInflater)
        setContentView(mActivityInterstitialTest3Binding.root)
        ...
        if (mInterType == R.id.rbInterOnCreate) {
            GoogleInterstitialAds().show(this@InterstitialTest3Activity, getInterAdsUnitId()) {
                // noting to do
            }
        }
        mActivityInterstitialTest3Binding.btnShowInterstitial1.setOnClickListener { testInterstitial(mInterType, 1) }
        mActivityInterstitialTest3Binding.btnShowInterstitial2.setOnClickListener { testInterstitial(mInterType, 2) }
    ...
    private fun testInterstitial(mInterType: Int, clickPos: Int) {
        if (mInterType == R.id.rbInterOnClick) {
            GoogleInterstitialAds().show(this@InterstitialTest3Activity, getInterAdsUnitId()) { startInterTestAct(mInterType, clickPos) }
        } else {
