...
import com.origin.ads.sample.databinding.ActivityInterstitialTest1Binding
import com.origin.ads.sample.utils.getInterAdsUnitId

class InterstitialTest1Activity : AppCompatActivity() {
    private lateinit var mActivityInterstitialTest1Binding: ActivityInterstitialTest1Binding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mActivityInterstitialTest1Binding = ActivityInterstitialTest1Binding.inflate(layoutInflater)
        setContentView(mActivityInterstitialTest1Binding.root)
        ...
        if (mInterType == R.id.rbInterOnCreate) {
            GoogleInterstitialAds().show(this@InterstitialTest1Activity, getInterAdsUnitId()) {
                // noting to do
            }
        }
        mActivityInterstitialTest1Binding.btnShowInterstitial2.setOnClickListener { testInterstitial(mInterType, 2) }
        mActivityInterstitialTest1Binding.btnShowInterstitial3.setOnClickListener { testInterstitial(mInterType, 3) }
    ...
    private fun testInterstitial(mInterType: Int, clickPos: Int) {
        if (mInterType == R.id.rbInterOnClick) {
            GoogleInterstitialAds().show(this@InterstitialTest1Activity, getInterAdsUnitId()) { startInterTestAct(mInterType, clickPos) }
        } else {
